...
MVP-0.2 已从“基础 MVP”推进到可试用 Beta：支持 Docker 三服务部署、命令行用户初始化、实验列表筛选分页、新建三入口、clone 来源提示、编辑器扩展字段、提交前结构化验证、样品/文件/审计/导出和受控词表管理。
...
- 实验列表支持 mine/status/q 筛选和分页。
- 详情页显示 clone 来源，并支持状态动作、样品概览、文件概览、审计轨迹和导出入口。
...
- gas component 写回采用后端约定的 `name/fraction`，读取兼容旧的 `gas/ratio_percent`。
...
- 不在 MVP-0.2 内引入完整字段字典和动态 schema 引擎。
...
- `cd backend && uv run pytest`：`107 passed`
- `cd frontend && bun run lint`
- `cd frontend && bun run typecheck`
- `cd frontend && bun run test`：`15 passed / 66 passed`
