...
describe("resolveErrorMessage", () => {
  ...
  it("resolves HTTP 401 status code to friendly Chinese error", () => {
    ...
    expect(resolveErrorMessage(errorUnauthorized, "默认错误")).toBe("登录会话已过期，请重新登录");
    expect(resolveErrorMessage(errorInvalidCreds, "默认错误")).toBe("邮箱或密码错误");
    expect(resolveErrorMessage(errorInactive, "默认错误")).toBe("该账号已被禁用，请联系管理员");
  });
  ...
  it("resolves HTTP 5xx status codes to friendly Chinese error", () => {
    ...
    const errorCustom = new HttpError(500, "OperationalError: postgres connection failed", null);
    expect(resolveErrorMessage(error500, "默认错误")).toBe("服务器内部错误，请稍后重试");
    expect(resolveErrorMessage(error503, "默认错误")).toBe("服务器内部错误，请稍后重试");
    expect(resolveErrorMessage(errorCustom, "默认错误")).toBe("服务器内部错误，请稍后重试");
  });
  ...
  it("resolves network errors to friendly Chinese error", () => {
    ...
    expect(resolveErrorMessage(fetchErr, "默认错误")).toBe("网络连接失败，请检查网络设置或稍后重试");
    expect(resolveErrorMessage(networkErr, "默认错误")).toBe("网络连接失败，请检查网络设置或稍后重试");
    expect(resolveErrorMessage(failedErr, "默认错误")).toBe("网络连接失败，请检查网络设置或稍后重试");
  });

  it("filters out JS runtime errors and shows fallback", () => {
    const typeErr = new TypeError("Cannot read properties of undefined (reading 'status')");
    const refErr = new ReferenceError("x is not defined");
    const syntaxErr = new SyntaxError("Unexpected token");
    expect(resolveErrorMessage(typeErr, "操作失败，请重试")).toBe("操作失败，请重试");
    expect(resolveErrorMessage(refErr, "操作失败，请重试")).toBe("操作失败，请重试");
    expect(resolveErrorMessage(syntaxErr, "操作失败，请重试")).toBe("操作失败，请重试");
  });
