...
function readInheritancePayload(
  sourceExperimentId: string,
): InheritanceStoragePayload | null {
  if (typeof window === 'undefined') {
    return null
  }

  const rawPayload = window.sessionStorage.getItem(
    inheritanceStorageKey(sourceExperimentId),
  )
  if (!rawPayload) {
    return null
  }
...
function buildInheritedValues({
  ...
  if (environment) {
    modulePayloads.push(
      createInheritedModulePayload(experiment.id, 'environment', environment),
    )
  }
  if (precheck) {
    modulePayloads.push(
      createInheritedModulePayload(experiment.id, 'precheck', precheck),
    )
  }
...
function groupValidationErrorsBySection(errors: EditorValidationError[]) {
  return errors.reduce<Map<EditorSectionKey, EditorValidationError[]>>(
    (result, error) => {
...
function shouldShowValidationResult(validation: ExperimentValidationResponse) {
  return (
    ...
    (typeof validation.completion_score === 'number' &&
      validation.completion_score < 100)
  )
}

function toCharacterizationCompletionPayload(
  values: ExperimentEditorValues['characterization'],
) {
  return {
...
export function useExperimentEditor({
  ...
  const pendingInheritedSectionsRef = useRef<Set<InheritedSectionKey>>(
    new Set(),
  )
  ...
  const [sectionStates, setSectionStates] = useState<
    Record<EditorSectionKey, SectionSaveState>
  >(createInitialSectionStates)
  ...
  const [validationResult, setValidationResult] =
    useState<ExperimentValidationResponse | null>(null)
  ...
  const getDirtySections = useCallback(
    (draftValues: ExperimentEditorValues) => {
      return editorSectionKeys.filter(
        (sectionKey) =>
          serializeSectionValues(sectionKey, draftValues) !==
          snapshotsRef.current[sectionKey],
      )
    },
    [],
  )
  ...
  const consumeInheritanceIfComplete = useCallback(() => {
    const inheritedSourceId =
      inheritedSourceIdRef.current ?? inheritFrom ?? null
      ...
      queryClient.setQueryData<ExperimentModulePayloadListResponse>(
        ...
        (current) => {
          ...
          const nextItems = currentItems.some(
            (item) => item.module_key === savedModule.module_key,
          )
  ...
  const replaceSetupMethodsSnapshot = useCallback(
    (savedSetupMethods: SetupMethodsRead, requestSectionSnapshot?: string) => {
      ...
      const shouldReplaceValues =
        requestSectionSnapshot === undefined ||
        currentSectionSnapshot === requestSectionSnapshot
      const nextValues = {
        ...valuesRef.current,
        setupMethods,
      }
  ...
  const persistDirtySections = useCallback(
    async (
      ...
      const validationErrorsBySection =
        groupValidationErrorsBySection(validationErrors)
      ...
      for (const sectionKey of savableSections) {
        ...
        let savedSetupValues: ExperimentEditorValues['setupMethods'] | null =
          null
        ...
        try {
          if (sectionKey === 'setup_methods') {
            ...
            savedSetupValues = replaceSetupMethodsSnapshot(
              response.data,
              requestSectionSnapshot,
            )
            ...
            nextExperiment = {
              ...
              summary_result:
                draftValues.resultSummary.summaryResult.trim() || null,
            }
          ...
          if (savedSetupValues) {
            setSectionState(sectionKey, {
              status: 'saved',
              message: formatSaveMessageWithWarnings(
                '已自动保存',
                setupWarnings,
              ),
            })
  ...
  const clearInheritedSection = useCallback(
    (sectionKey: InheritedSectionKey) => {
    ...
    const applyInheritance = async () => {
      try {
        ...
        if (!environment && !precheck) {
          const sourceModules = await listExperimentModules(
            accessToken,
            inheritFrom,
          )
          const sourcePayloads = Object.fromEntries(
            sourceModules.items.map((module) => [
              module.module_key,
              module.payload_json,
            ]),
          )
        ...
        const nextValues: ExperimentEditorValues = {
          ...valuesRef.current,
          environment:
            inheritedValues.environment ?? valuesRef.current.environment,
        ...
        setValidationResult(null)
        setInheritedFrom({
          ...(inheritedValues.environment
            ? { environment: sourceRunCode }
            : {}),
  ...
  const saveSummary = useMemo(() => {
    ...
    const errorCount = summaryStates.filter(
  ...
  const currentModulePayloads = useMemo<
  ...
  >(
    () => ({
      ...
      process_observation: toProcessObservationPayload(
        values.processObservation,
      ),
      characterization: toCharacterizationCompletionPayload(
  ...
  const diffModulePayloads = useMemo<
    Record<ModuleEditorSectionKey, Record<string, unknown>>
  >(
    () => ({
      ...currentModulePayloads,
      characterization: mergeCharacterizationPayload(
        undefined,
        values.characterization,
      ),
    }),
  ...
  const completionPayloads = useMemo<
    Record<EditorSectionKey, Record<string, unknown>>
  >(
    () => ({
      ...currentModulePayloads,
  ...
  const completionValidationIssues = useMemo<
  ...
  const moduleCompletionMap = useMemo<
    Record<EditorSectionKey, ModuleCompletionStatus>
  >(
    () =>
      editorSectionKeys.reduce<
        Record<EditorSectionKey, ModuleCompletionStatus>
      >(
        (result, sectionKey) => {
  ...
  const completionSummary = useMemo<CompletionSummary>(() => {
    const statuses = editorSectionKeys.map(
      (sectionKey) => moduleCompletionMap[sectionKey],
    )
    const totalCount = statuses.length
    const frontendPercent = Math.round(
      statuses.reduce((total, status) => total + status.percent, 0) /
        totalCount,
    )
    ...
    return {
      percent:
        typeof validationResult?.completion_score === 'number'
          ...
          : frontendPercent,
      completedCount: statuses.filter((status) => status.state === 'complete')
        .length,
      totalCount,
      blockingCount: validationResult
        ...
        : statuses.reduce(
            (total, status) =>
              total + (status.state === 'error' ? status.errors : 0),
            0,
          ),
      warningCount: validationResult
        ...
        : statuses.reduce(
            (total, status) =>
              total + (status.state === 'warning' ? status.warnings : 0),
            0,
          ),
    }
  ...
  const submitDraft = useCallback(async () => {
    ...
    try {
      ...
      const submittedExperiment = await submitExperiment(
        accessToken,
        experimentId,
      )
