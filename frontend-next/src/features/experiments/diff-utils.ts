...
export type ModulePayloadDiffInput = {
  sourceModules: Partial<
    Record<ModuleEditorSectionKey | string, Record<string, unknown>>
  >
  currentModules: Partial<
    Record<ModuleEditorSectionKey | string, Record<string, unknown>>
  >
}
...
export function getFieldLabel(path: string) {
  ...
  return parts
    .map((part, index) => normalizePathPart(part, parts[index + 1]))
    .join(' / ')
}
...
export function buildExperimentModuleDiffs({
  ...
  return moduleEditorSectionKeys
    .filter(
      (moduleKey) => sourceModules[moduleKey] || currentModules[moduleKey],
    )
    .map((moduleKey) => {
