...
function getDeclaredFurnaceZoneKeys(payload: Record<string, unknown>) {
  ...
  const zonesCount =
    typeof rawZonesCount === 'number'
      ? rawZonesCount
      : Number(String(rawZonesCount ?? '').trim())
  ...
  return Array.from(
    { length: Math.floor(zonesCount) },
    (_, index) => `zone_${index + 1}`,
  )
}
...
function furnaceZoneHasValidTemperatureProgram(zone: Record<string, unknown>) {
  ...
  for (const node of nodes) {
    if (
      !isNonNegativeNumberLike(node.time_min) ||
      !isFilled(node.temperature_C)
    ) {
      return false
    }
...
function baseCompletion(moduleKey: string, payload: Record<string, unknown>) {
  ...
  if (moduleKey === 'setup_methods') {
    const setupNameSnapshot = getValue(payload, [
      'setup_name_snapshot',
      'setupNameSnapshot',
    ])
    const setupKeySnapshot = getValue(payload, [
      'setup_key_snapshot',
      'setupKeySnapshot',
    ])
    const methodsTextSnapshot = getValue(payload, [
      'methods_text_snapshot',
      'methodsTextSnapshot',
    ])
    const diagramFileAssetId = getValue(payload, [
      'diagram_file_asset_id',
      'diagramFileAssetId',
    ])
    const referencePaperUrlSnapshot = getValue(payload, [
      'reference_paper_url_snapshot',
      'referencePaperUrlSnapshot',
    ])
    const unpublishedReasonSnapshot = getValue(payload, [
      'unpublished_reason_snapshot',
      'unpublishedReasonSnapshot',
    ])
    const isSameAsSource = getValue(payload, [
      'is_same_as_source',
      'isSameAsSource',
    ])
    const deviationNote = getValue(payload, ['deviation_note', 'deviationNote'])
    const sourceSetupLibraryId = getValue(payload, [
      'source_setup_library_id',
      'sourceSetupLibraryId',
    ])
    ...
    const requiredFields = [
      ...
      isFilled(referencePaperUrlSnapshot) || isFilled(unpublishedReasonSnapshot)
        ? 'reference'
        : '',
      !hasSource || isSameAsSource === true || isFilled(deviationNote)
        ? 'deviation'
        : '',
    ]
  ...
  if (moduleKey === 'precursors') {
    ...
    return items.every(
      (item) => isFilled(item.species) && isFilled(item.method),
    )
      ? 100
      : 50
  }
  ...
  if (moduleKey === 'substrates') {
    ...
    return items.every((item) => isFilled(item.type) && isFilled(item.role))
      ? 100
      : 50
  }
  ...
  if (moduleKey === 'furnace_program') {
    ...
    const zoneByKey = new Map(
      zones.map((zone) => [String(zone.zone_key ?? ''), zone]),
    )
    const hasAllZones =
      expectedZoneKeys.length > 0 &&
      expectedZoneKeys.every((zoneKey) => zoneByKey.has(zoneKey))
    const hasValidPrograms =
      hasAllZones &&
      expectedZoneKeys.every((zoneKey) =>
        furnaceZoneHasValidTemperatureProgram(zoneByKey.get(zoneKey)!),
      )
  ...
  if (moduleKey === 'gas_program') {
    ...
    return segments.every((segment) => {
      ...
      return (
        components.length > 0 &&
        components.some((c) => isPositiveNumberLike(c.flow_sccm))
      )
    })
      ? 100
      : 50
  ...
  if (moduleKey === 'characterization') {
    return asRecordArray(payload.methods).some(
...
function countFrontendWarnings(
  ...
  if (moduleKey === 'environment') {
    const temperature = getValue(payload, [
      'indoor_temperature_C',
      'indoorTemperatureC',
    ])
    const humidity = getValue(payload, [
...
function issuesForModule(
  moduleKey: string,
  validationIssues: CompletionValidationIssue[],
) {
...
export function computeModuleCompletion(
  ...
  if (hasApiIssues) {
    const errorCount = moduleIssues.filter(
