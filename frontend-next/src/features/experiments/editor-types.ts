...
export const moduleEditorSectionKeys = editorSectionKeys.filter(
  (sectionKey): sectionKey is ModuleEditorSectionKey =>
    sectionKey !== 'setup_methods',
)
...
export type ModulePayloadMap = Partial<
...
export function resolvePrecursorMethodFlags(
  method: string,
): PrecursorMethodFlags {
  const normalizedMethod = method.trim().toLowerCase()
  const isSolutionOrSpin =
    normalizedMethod === 'solution' || normalizedMethod === 'spin_coating'
...
export function createPrecursorMethodPatch(
  method: string,
): Partial<PrecursorItemValues> {
...
export function validateSectionValues(
  ...
  if (sectionKey === 'furnace_program') {
    const zonesCount = parsePositiveIntegerValue(
      values.furnaceProgram.furnaceInfo.zonesCount,
    )
    if (zonesCount === null) {
      errors.push({
        sectionKey,
    ...
    const allowedZoneKeys = new Set(
      getValidFurnaceZoneKeys(values.furnaceProgram.furnaceInfo.zonesCount),
    )
...
export function shouldSanitizeSubstratesBeforeSubmit(values: SubstratesValues) {
  return values.items.some((item) => {
    ...
    return (
      !isFixedRole ||
      (hasSourcePayload(item.sourcePayload) && !hasSubstrateEditableValue(item))
    )
  })
}
...
export function segmentsToTemperatureNodes(
  ...
  const nodes: {
  ...
  }[] = [
    {
      ...
      time_min: 0,
      temperature_C:
        startTemp !== null && Number.isFinite(startTemp) ? startTemp : null,
      note: '',
    },
  ]
...
function inferFurnaceSegmentLabel(
  ...
  const prevTemperature =
    prevNode.temperature_C != null ? Number(prevNode.temperature_C) : null
  const currentTemperature =
    node.temperature_C != null ? Number(node.temperature_C) : null
  if (
...
export function temperatureNodesToSegments(
  ...
  const startTemperatureC =
    startNode.temperature_C != null ? String(startNode.temperature_C) : ''

  if (nodes.length === 1) {
    return {
      startTemperatureC: startTemperatureC || '25',
      segments: createDefaultZoneSegments(),
    }
  }
  ...
  const segments: FurnaceSegmentValues[] = nodes.slice(1).map((node, index) => {
    ...
    const prevTime =
      prevNode.time_min != null ? Number(prevNode.time_min) : null
    ...
    const dur =
      ...
      Number.isFinite(curTime)
        ...
        : null
    return {
      label: inferFurnaceSegmentLabel(
        node,
        prevNode,
        defaultLabels[index],
        index,
      ...
      targetTemperatureC:
        node.temperature_C != null ? String(node.temperature_C) : '',
...
export function payloadToFurnaceProgramValues(
  ...
  const declaredZonesCount = parsePositiveIntegerValue(
    asString(info.zones_count),
  )
  ...
  const existingZonesByKey = new Map(
    rawZoneValues.map((zone) => [zone.zoneKey, zone]),
  )
  const zones: FurnaceZoneValues[] = declaredZoneKeys.map(
    (zoneKey) =>
      existingZonesByKey.get(zoneKey) ?? createEmptyFurnaceZone(zoneKey),
  )
  const zonesCount =
    declaredZonesCount !== null
      ? String(declaredZonesCount)
      : String(declaredZoneKeys.length)

  const placements: FurnacePlacementValues[] = asObjectArray(
    furnaceProgram.placements,
  ).map((p) => ({
...
export function syncFurnaceProgramZonesCount(
  ...
  return {
    ...
    zones: newZoneKeys.map(
      (k) => existingByKey.get(k) ?? createEmptyFurnaceZone(k),
    ),
  }
}
...
export function createInitialSectionStates(): Record<
  EditorSectionKey,
  SectionSaveState
> {
  return {
...
export function createModulePayloadMap(
  items: ExperimentModulePayloadRead[],
): ModulePayloadMap {
  return items.reduce<ModulePayloadMap>((result, item) => {
    if (
      !moduleEditorSectionKeys.includes(
        item.module_key as ModuleEditorSectionKey,
      )
    ) {
      return result
    }

    result[item.module_key as ModuleEditorSectionKey] = asRecord(
      item.payload_json,
    )
    return result
  }, {})
...
export function createSetupMethodsValues(
  ...
  return {
    ...
    apparatusDescriptionSnapshot:
      snapshot?.apparatus_description_snapshot ?? '',
    ...
    reactionFlowDescriptionSnapshot:
      snapshot?.reaction_flow_description_snapshot ?? '',
    ...
    semanticContextText: JSON.stringify(
...
export function createInitialEditorValues(
  ...
  return {
    basicInfo: {
      experimentType:
        asString(basicInfo.experiment_type) ||
        asString(experiment.experiment_type),
    ...
    precheck: {
      ...
      boatContaminationLevel: asNullableBooleanValue(
        precheck.boat_contamination_level,
      ),
      tubeContaminationLevel: asNullableBooleanValue(
        precheck.tube_contamination_level,
      ),
    },
    precursors: {
      items:
    ...
    substrates: {
      items: asObjectArray(substrates.items).map((item) => ({
        ...
        treatmentTemperatureC: asString(
          asRecord(item.treatment_params).temperature_C,
        ),
        treatmentDurationMin: asString(
          asRecord(item.treatment_params).duration_min,
        ),
    ...
    resultSummary: {
      summaryResult:
        asString(experiment.summary_result) ||
        asString(resultSummary.summary_result),
      qualityLabel:
...
export function toExperimentPatch(
...
export function toSetupMethodsPayload(
  ...
  return {
    ...
    sample_placement_description_snapshot:
      values.samplePlacementDescriptionSnapshot.trim(),
    reaction_flow_description_snapshot:
      values.reactionFlowDescriptionSnapshot.trim(),
    reference_paper_url_snapshot: normalizeNullableString(
      values.referencePaperUrlSnapshot,
    ),
    unpublished_reason_snapshot: normalizeNullableString(
      values.unpublishedReasonSnapshot,
    ),
...
export function toPrecheckPayload(values: PrecheckValues) {
  return {
    ...
    boat_contamination_level: normalizeNullableBoolean(
      values.boatContaminationLevel,
    ),
    tube_contamination_level: normalizeNullableBoolean(
      values.tubeContaminationLevel,
    ),
  }
}
      ...
      .map((item) =>
        mergePayloadFields(
          ...
          removeNullEntries({
            ...
            melting_temperature_C: normalizeNumberLike(
              item.meltingTemperatureC,
            ),
...
export function mergePrecursorsPayload(
  ...
  return mergePayloadFields(existingPayload, toPrecursorsPayload(values), [
    'items',
  ])
}
    ...
    items: values.items
      .filter(
        (item) =>
          ...
          hasSubstrateEditableValue(item),
      )
      .map((item) => {
        const existingTreatmentParams = asRecord(
          asRecord(item.sourcePayload).treatment_params,
        )
        ...
        return mergePayloadFields(
          ...
          removeNullEntries({
            ...
            treatment_params:
              Object.keys(nextTreatmentParams).length > 0 ||
              Object.keys(existingTreatmentParams).length > 0
                ? mergePayloadFields(
                    existingTreatmentParams,
                    nextTreatmentParams,
                    ['temperature_C', 'duration_min', 'power_W', 'gas'],
                  )
                : undefined,
          }),
          [
...
export function mergeSubstratesPayload(
  ...
  return mergePayloadFields(existingPayload, toSubstratesPayload(values), [
    'items',
  ])
}
...
export function toFurnaceProgramPayload(values: FurnaceProgramValues) {
  ...
  const initialTemperaturesC = Object.fromEntries(
    values.zones.map((zone) => [
      zone.zoneKey,
      normalizeNumberLike(zone.startTemperatureC),
    ]),
  )
  ...
  return {
    ...
    placements: values.placements
      ...
      .map((p) => {
        ...
        return {
          precursor_index: Number.isFinite(precursorIndex)
            ? precursorIndex
            : null,
    ...
    zones: values.zones.map((zone) => ({
      ...
      temperature_program: segmentsToTemperatureNodes(
        zone.startTemperatureC,
        zone.segments,
      ),
      note: zone.note,
      ...
      .map((segment) => {
        ...
        return {
          ...
          payload: (() => {
            ...
            const totalComponentFlow = validComponentFlows.reduce(
              (sum, v) => sum + v,
              0,
            )

            const computedSegmentFlow =
              componentPayloads.length > 0 && totalComponentFlow > 0
                ? totalComponentFlow
            ...
            for (const cp of componentPayloads) {
              ...
              if (typeof compFlow === 'number' && totalComponentFlow > 0) {
                cp.fraction =
                  Math.round((compFlow / totalComponentFlow) * 1000000) /
                  1000000
                cp.ratio_percent =
                  Math.round((compFlow / totalComponentFlow) * 10000) / 100
              } else {
                cp.fraction = null
            ...
            return mergePayloadFields(
              ...
              removeNullEntries({
                ...
                components:
                  componentPayloads.length > 0 ? componentPayloads : undefined,
              }),
              [
                'stage',
                'gas',
...
export function mergeProcessObservationPayload(
  ...
  return mergePayloadFields(
    existingPayload,
    toProcessObservationPayload(values),
    ['color_change', 'abnormal_events', 'note'],
  )
}
...
export function mergeCharacterizationPayload(
  ...
  return mergePayloadFields(
    existingPayload,
    toCharacterizationPayload(values),
    ['methods'],
  )
}

export function toResultSummaryPatch(
