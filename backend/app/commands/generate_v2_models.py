...
MATERIAL_PHASE_SOURCE = REPO_ROOT / "docs" / "standard" / "material-phase-catalog.yaml"
MATERIAL_PHASE_OUTPUT = REPO_ROOT / "backend" / "app" / "generated" / "material_phase_catalog.py"
...
def generate_material_phase_catalog(
    *,
    source_path: str | Path = MATERIAL_PHASE_SOURCE,
    output_path: str | Path = MATERIAL_PHASE_OUTPUT,
) -> str:
    doc = yaml.safe_load(Path(source_path).read_text(encoding="utf-8"))
    catalog = {
        formula: tuple(
            (phase["code"], phase["bulk_space_group_number"]) for phase in material["phases"]
        )
        for formula, material in doc["materials"].items()
    }
    target = Path(output_path)
    target.parent.mkdir(parents=True, exist_ok=True)
    target.write_text(
        ...
        "# ruff: noqa: E501\n"
        "# fmt: off\n\n"
        f"MATERIAL_PHASE_CATALOG = {catalog!r}\n",
        encoding="utf-8",
    )
    return str(target)
...
def render_v2_models(doc: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "    equipment_name: NonBlankStr | None = None",
        "    ultrasonic_frequency_kHz: Annotated[float, Field(allow_inf_nan=False, gt=0)] | None = None",
        "    ultrasonic_power_W: Annotated[float, Field(allow_inf_nan=False, gt=0)] | None = None",
        "    duration_min: Annotated[float, Field(strict=True, allow_inf_nan=False, gt=0)] | None = None",
        "",
        ...
        "        if self.wiping_material and self.cleaning_method != 'wipe':",
        '            raise ValueError("wiping_material applies only to wiping")',
        "        if self.cleaning_method != 'ultrasonic' and any(value is not None for value in (self.ultrasonic_frequency_kHz, self.ultrasonic_power_W)):",
        '            raise ValueError("ultrasonic parameters apply only to ultrasonic cleaning")',
        ...
        "",
        "",
        "class UvOzoneParametersPayload(RequiredDurationParametersPayload):",
        "    mode: Literal['uv_ozone', 'uv_only', 'ozone_only'] | None = None",
        "    equipment_name: NonBlankStr | None = None",
        "    source_distance_mm: Annotated[float, Field(allow_inf_nan=False, gt=0)] | None = None",
        "    atmosphere: NonBlankStr | None = None",
        "    temperature_C: Annotated[float, Field(allow_inf_nan=False, gt=-273.15)] | None = None",
        "    wavelength_nm: Annotated[float, Field(allow_inf_nan=False, gt=0)] | None = None",
        "    irradiance_mW_cm2: Annotated[float, Field(allow_inf_nan=False, gt=0)] | None = None",
        "    ozone_concentration_ppm: Annotated[float, Field(allow_inf_nan=False, gt=0)] | None = None",
        "    gas_flow_sccm: Annotated[float, Field(allow_inf_nan=False, gt=0)] | None = None",
        "",
        '    @model_validator(mode="after")',
        "    def _mode_parameters(self) -> Self:",
        "        if self.mode == 'ozone_only' and any(value is not None for value in (self.source_distance_mm, self.wavelength_nm, self.irradiance_mW_cm2)):",
        '            raise ValueError("UV parameters do not apply to ozone-only treatment")',
        "        if self.mode == 'uv_only' and self.ozone_concentration_ppm is not None:",
        '            raise ValueError("ozone concentration does not apply to UV-only treatment")',
        "        return self",
        "",
        "",
        "class UvOzonePretreatmentPayload(V2PayloadBase):",
        "    type: Literal['uv_ozone_treatment']",
        "    parameters: UvOzoneParametersPayload",
        "",
        "",
        ...
        "    capability_name: NonBlankStr | None = None",
        "    start_min: Annotated[float, Field(strict=True, allow_inf_nan=False, ge=0)]",
        "    end_min: Annotated[float, Field(strict=True, allow_inf_nan=False, gt=0)]",
        "    parameters: Annotated[list[NamedParameterPayload], Field(min_length=1)]",
        "",
        ...
        '            raise ValueError("capability_name applies only to custom capabilities")',
        "        aliases = {",
        "            'plasma': {",
    ...
    for field in composite_fields.values():
        ...
        required_value_options = (field.get("validation") or {}).get("require_value_options")
        if required_value_options:
            lines.extend(
                [
                    "",
                    '    @model_validator(mode="after")',
                    "    def _requires_value_for_option(self) -> Self:",
                    f"        if self.option in {tuple(required_value_options)!r} and self.value is None:",
    ...
    if "field_device_other_names" in keys:
        lines.extend(
            [
                "",
                f'{indent}@model_validator(mode="after")',
                f"{indent}def _additional_capabilities(self) -> Self:",
                f"{indent}    names = self.field_device_other_names",
                f"{indent}    if names is not None:",
                f"{indent}        names = [name.strip() for name in names]",
                f"{indent}        if any(not name or len(name) > 128 for name in names) or len({{name.casefold() for name in names}}) != len(names):",
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(generate_material_phase_catalog())
    return 0
