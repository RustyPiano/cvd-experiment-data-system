...
| `temperature_C` | 处理温度 | Treatment Temperature | number | ℃ |  |  |  |
| `duration_min` | 处理时长 | Treatment Duration | number | min |  |  |  |
| `power_W` | 处理功率 | Treatment Power | number | W |  |  |  |
| `gas` | 处理气体 | Treatment Gas | text |  |  |  |  |
