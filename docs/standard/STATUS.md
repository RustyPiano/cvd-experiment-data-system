...
- 当前阶段：**炉次优先阶段 0–4 与 2026-07-24 全项目审查整改均已完成**。本轮进一步收紧科学组成、复合缺失语义、真实 PostgreSQL 并发与一致性快照、实体附件证据、生产配置、备份/部署 fail-closed 及用户展示；最新整改矩阵见 `docs/reviews/2026-07-24-comprehensive-audit-remediation.md`，验收证据见 `docs/operations/e2e-comprehensive-hardening-report-2026-07-24.md`。下一步仍为 **push + Actions 首绿 → 批8生产切换人工门**。
...
7. **发布硬顺序**：push → GitHub Actions 首绿（含 PostgreSQL smoke）→ 设置 required checks → **批8生产切换（人工门，用户在场）**：预构建 → 停写但保留容器 → 数据库/存储双备份与恢复演练 → 当日删除确认 → 重建空库与清空专用卷 → verified capability 部署 → `create_admin` → 线上冒烟。**批8完成前禁止运行 `deploy.sh`**。
