...
> 任何文档与本文件冲突，**以本文件为准**。最后更新：2026-07-09。

## 0. 一分钟速览（给接手的 Agent）
- 这是一个 **CVD 二维材料实验数据采集系统**。
- **线上运行 = v1**（`cvd_v1` / 68 字段），但 **2026-07-09 已定案：v1 试运行数据可弃、完全拆除 v1、v2 单轨化**——拆除+重基线计划见 **`docs/v2-single-track-plan.md`**（批0–批7，取代原 P5 迁移/P6 切换；**迁移全线作废**）。v2 代码已开发完成（P0–P5 工具，见 §5）。
- 当前阶段：两线并行——**线 A** 单轨化拆除（批0 备份后即可开工）；**线 B** 等俊杰对齐 4 问 → P1.5 冻结（互不阻塞，见 §6）。
...
3. **P1.5 冻结仪式**：标准头 `DRAFT`→`FROZEN`、YAML meta.status→FROZEN → **push（Actions 首绿）** → tag `v2.0.0`。

> 悬而未决小项：化学式渲染规则组内确认（待明确#1）。（原 `list_runs` 100 条上限项已并入线 A 批3。）
