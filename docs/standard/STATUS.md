...
> 任何文档与本文件冲突，**以本文件为准**。最后更新：2026-07-23。
...
- 当前阶段：**炉次优先工作流与全量双语文案重构已完成阶段 0–4、真实浏览器 E2E 和最终全库复核**。当前未提交工作树进一步收紧录入契约：实验人固定为创建者，受控词表改为 ASCII code + 双语显示，数值/复合/多值字段结构化，化学式与 IT 空间群号强校验。**独立 Agent 前两轮审核的 7 项 + 3 项问题均已修复，最终独立终审未发现 P0/P1/P2，自动化门禁全绿，待提交**。之后顺序仍为 **push + Actions 首绿 → 批8生产切换人工门**。
- 两份交付物已 **freeze-ready**（经 **3 轮独立评审**，最终 8.5/10）：
  - 字段表 `字段草案-v3.xlsx`（**v3.5**，77 字段 + 3 张一等实体表）——生成脚本 `build_field_tables.py`
...
- **门禁：后端 pytest 181/181 · 前端 vitest 232/232 · ruff/format/tsc/eslint/build/文本生成物确定性/字段源全绿（本地）；OpenAPI 已重新生成；全部 commit 未 push（push 时机见 §6）**。
...
| 07-22 | **录入契约收紧**：实验人改为当前登录姓名自动填入且不可改写；“化学体系/化学式”改为“目标材料化学式”，补齐六类结构的填写规则，化学式统一使用 ASCII 元素符号+普通数字并由后端校验；体相空间群改为 IT number `1–230`。所有受控选项改为稳定 ASCII code 存储、中英文显示，旧中文输入在边界归一化；普通数值存 JSON number，复合字段存 `{value, option}`，多条气体存数组，导出同步关系化。同步生成后端模型、JSON Schema、OpenAPI 类型、前端元数据、字段字典和 xlsx。**独立 Agent 首轮指出的严格数值/空间群、全导出归一化、气体数组回填、复合值双语显示、化学式一致性、旧结果编辑归一化和首选标签覆盖 7 项均已修复；pytest 181/181 · vitest 231/231 及静态/生成门禁全绿**。 | 本次提交 |
| 07-23 | **录入契约第二轮独立复审修复**：通用多值表单状态改为数组，气体组分改用可访问复选控件，支持 `Ar + H2` 中英显示、payload 回显与再保存无损；CSV 所有 nested leaves 在关系化前统一递归归一化，覆盖仪器快照、表征属性、实测属性与独立表征分支；生成器隐藏字段测试改用真实 `unload` discriminator，先证明合法再验越域拒绝，消除假绿。**pytest 181/181 · vitest 232/232 及全门禁绿；最终独立终审未发现 P0/P1/P2，结论可接受**。 | 本次提交 |
...
5. **阶段 4 已完成**：新版 17 项浏览器 E2E 全部通过，阶段独立复审问题已闭环；报告见 `docs/operations/e2e-run-first-report-2026-07-17.md`。
6. **最终全库复核已完成**：所有发现均已修复并由独立 Agent 复核关闭；后端、前端、生成物和字段源全门禁已重跑通过。
7. **发布硬顺序**：push → GitHub Actions 首绿 → 设置 required checks → **批8生产切换（人工门，用户在场）**：停容器 → 重建生产库 → 部署 → `create_admin` → 线上冒烟。**批8完成前禁止运行 `deploy.sh`**。
