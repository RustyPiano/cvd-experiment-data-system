...
> 任何文档与本文件冲突，**以本文件为准**。最后更新：2026-07-24。
...
- **香港生产已于 2026-07-24 切换到 v2**：初始切换目标 `4e0b65a` 与后续发布的五项 required checks 全绿，backend/frontend 健康，管理员 API 与真实浏览器登录通过；当前应用发布提交为 `57a25b7`。旧 v1 数据没有删除，已离线归档为 `cvd_v1_archive_20260724`；证据见 `docs/operations/production-deployment-report-2026-07-24.md`。
- 当前阶段：**2026-07-24 导师线上走查整改已完成代码、最终全门禁、独立终审、真实主线验收，并经 PR #1 发布香港生产**。最终未闭环 P0/P1 = 0；下一步是使用本轮报告向大导汇报并确认 11 项专业标准。
- 两份标准交付物已更新到本轮 **review-ready** 状态：
  - 字段表 `字段草案-v3.xlsx`（**字段修订 v3.7**，84 个实验字段 + 3 张一等实体表共 44 字段，其中 30 个字段标记为 R0）——生成脚本 `build_field_tables.py`
  - 文字标准 `cvd-2d-process-data-standard-v2.0.md`
- **导师评审输入**：2026-07-07 书面批注已纳入 v3.4；2026-07-24 线上逐项走查已形成 `docs/product/2026-07-24-meeting-remediation-plan.md`，逐项状态与汇报证据见 `docs/reviews/2026-07-24-teacher-meeting-remediation.md`。仍需组内裁定的专业词表和参数模板见 §6 线 B，工程实现不自行猜定。
...
- **v2 单轨形态**：已发布 Alembic 基线 `20260711_0001`（14 个应用表，SQLite/PG 双兼容）；本轮不改数据库结构、不新增迁移。`/api/v1` 继续使用 `draft → locked` 状态机与既有实体版本、样品、结果、文件和审计表；本轮把表格式自由输入收敛为强类型结构、冻结引用与附件绑定。
...
- **本地门禁：后端 Ruff/format 全绿，pytest 467 通过、4 条 PostgreSQL-only 跳过；前端 Prettier/ESLint/TypeScript/build 全绿，vitest 358/358（52 files）；字段源 84 个实验字段 + 44 个实体字段，其中 30 个字段标记为 R0，Pydantic/JSON Schema/OpenAPI/TypeScript 元数据/xlsx 已重生成并通过漂移校验。运行时 `check-r0` 另含跨实体和结构合规判据，共 44 项；一次性本地炉次 `CVD-2026-0001` 适用 43 项且 43/43 通过，并已完成创建—锁定—样品—结果/附件—JSON/ZIP 导出。**
- **装置图已上线**：沿用装置版本中的单张 `setup_diagram` 附件；图片在上传表单和版本详情中直接预览，在装置库列表显示缩略图，非图片仍保留文件名与下载入口。炉次引用的装置版本快照语义不变。
- **审查结论**：M-01—M-24、A-01—A-09 及最终收口审计 F-01—F-12 的工程整改均已关闭，未闭环 P0/P1 = 0。本轮工程整改与生产发布均已完成，剩余只有向大导汇报并确认 11 项专业标准。
- 工程约定见根 `AGENTS.md`。

## 2. 真相三件套（+ 字段表怎么改）
1. **`field-source.yaml`** —— **字段单一权威源**（P1 起生效，实现方案 D1）：84 个实验字段 + 44 个实体字段 + 条件必填表达式 + 30 个 R0 字段标记 + pending-alignment 标记。`字段草案-v3.xlsx` 是它的**渲染产物**（人读视图，4 个 sheet），由 `build_field_tables.py` 生成。导师批注原件在 `docs/research/FSS Re-副本字段草案-v3.xlsx`（勿改，输入件）。
...
版本口径：`2.0.0` 是标准语义版本，`cvd_v2` 是载荷标识，`v3.7` 是当前字段表内部修订号；沿用文件名 `字段草案-v3.xlsx` 不代表第三套 schema。
...
8. **本轮整改、验收与生产发布已完成**：执行矩阵见 `docs/product/2026-07-24-meeting-remediation-plan.md`，逐项汇报和本地主线证据见 `docs/reviews/2026-07-24-teacher-meeting-remediation.md`；香港生产当前应用提交为 `57a25b7`，发布证据见 `docs/operations/production-deployment-report-2026-07-24.md`。
9. **下一步**：按整改报告向大导汇报工程问题已关闭、未闭环 P0/P1 = 0，并请导师/俊杰/实际实验人裁定 11 项专业标准。
