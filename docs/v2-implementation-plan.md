...
> ⚠️ **2026-07-09 起部分取代，2026-07-11 单轨化已执行完毕**：用户确认 v1 试运行数据可弃后，**P5（迁移）作废、P6（切换）升级为完全拆除**，工程路线由 **`docs/v2-single-track-plan.md`**（v1 单轨化：批0–批8）取代并已执行至批7（仓库现为纯 v2 单轨，仅剩批8 生产切换人工门）；D9 的 additive-only 约束随 v1 拆除失效。P0–P4 的决策记录（D1–D12）与验收门仍为有效历史与技术依据。
>
> **状态：HISTORICAL（P0–P4 决策记录，工程路线已完结）** | 制定：2026-07-08 | 依据：`docs/standard/STATUS.md`（现状）+ 2026-07-08 v1 代码库全量摸底
>
...
| D12 | **国际化 i18n** | 分层：①字段层双语 = YAML `label`(中文,权威)+`label_en` **全量必填**（护栏强制）；②词表英文随 P3 结构化补齐；③前端 **P4 起即用 i18next 架构**（zh 默认 locale，字段标签目录由生成器⑤双语输出，UI 文案进 locale 文件不许硬编码）；④英文 UI 全量打磨 = 上线后事项，**不阻塞 v2.0**；xlsx 人读视图保持中文 | P4 组件里硬编码中文文案 = 跑偏；新字段缺 label_en = CI 红 |
...
5. 词表结构化（options 拆受控列表，**含每项英文名**，此时才动 YAML 结构）；化学式/异质结显示串渲染规则=纯函数+测试（关闭待明确 #1）
- ✅ 验收：契约测试绿 · 样例 run 的 R0 报告含全部条件必填用例 · OpenAPI 可 `gen:api`

### P4 前端表单（5–8 天，最大块）
1. **生成器⑤**：TS 字段元数据模块（label 双语/单位/选项/必填/条件显隐）+ "前端字段键 ⊆ schema"漂移测试进 CI；**i18next 脚手架**（zh 默认，UI 文案全走 locale 文件，D12）
...
4. **（择机，P6 验证稳定后）v1 退役清理 migration**：删 v1-only 废弃列/模块（`quality_label` 等），独立 commit 可回滚——消除 D9 复用策略遗留的闲置列，长期维护成本归零
...
- **2026-07-08 · P2–P5 按编排模式全部完成**：P2 `01790cc` · P3 `f93d467` · P4 三步 `92370b4/81b09b3/74c9720/18f4864` + 验收回归 `b0a5b3e` · P5 工具 `814725c`；各阶段验收门全部达成（P4 的"手工录入"由 API 级永久回归测试 `test_v2_full_walkthrough` 覆盖主体，浏览器走查列入 STATUS §6.5）。随后完成 4 组并行代码精简审查（43 findings，批1–3 待执行，报告 `docs/reviews/2026-07-08-simplify-review.md`）。剩余步骤队列以 **STATUS §6 为准**。
- **2026-07-08 · 开发执行模式 = 多智能体编排（用户授权）**：分工——**Codex(xhigh)**：后端实现/复杂逻辑/代码审查初稿；**Opus**：前端设计与 UI 实现；**Sonnet**：机械任务（测试补写/类型消费/样板代码）；**Fable(主会话)**：总编排/本机验证/门禁执行/每阶段 commit。**分阶段发射**（P2→验收→P3→验收→P4→P5工具），不一发全包；每阶段结束跑全门禁并 commit 后才进下一阶段。**两道人工门不变**：P5 正式迁移执行（备份确认）与 P6 生产切换必须用户在场。**§7 提前实现授权**：用户确认"先建后补"——§7 相关实现按当前词表先做，俊杰答案回来后走 YAML 补丁（预计 ≤ 半天），P1.5 冻结仪式照旧等真人对齐。
- **2026-07-08 · D10 命名标准化二轮 + D12 国际化（用户确认后补）**：①key 全词化改名 5 处（`tube_od_wall_mm`→`tube_outer_diameter_wall_mm` ×2 等），命名规则写进 YAML meta（全词优于缩写，保留 CAS/PID/MFC/CoA/D50）；②新增 D12：123 字段全量补 `label_en`（护栏强制非空），词表英文随 P3、i18next 随 P4、英文 UI 打磨不阻塞 v2.0；③P6 增"v1 退役清理 migration"择机项（消除 D9 复用的闲置列，长期成本归零）。
