...
> ⚠️ **2026-07-09 起部分取代，2026-07-11 单轨化已执行完毕**：用户确认 v1 试运行数据可弃后，**P5（迁移）作废、P6（切换）升级为完全拆除**，工程路线由 **`v2-single-track-plan.md`**（v1 单轨化：批0–批8）取代并已执行至批7（仓库现为纯 v2 单轨，仅剩批8 生产切换人工门）；D9 的 additive-only 约束随 v1 拆除失效。P0–P4 的决策记录（D1–D12）与验收门仍为有效历史与技术依据。
>
